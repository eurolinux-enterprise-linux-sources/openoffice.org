									...
									SCCOLROW* pBreak, SCCOLROW* pPrev )
{
    ...
    if (nFound != SC_PD_NONE)
    {
        ...
        {
            SCTAB nTab = pViewData->GetTabNo();
            BOOL bColumn = ( nFound == SC_PD_BREAK_H );
            if ( bColumn )
            {
                if (!( pDoc->GetColFlags( static_cast<SCCOL>(nBreak), nTab ) & CR_MANUALBREAK ))
                    return SC_PD_NONE;
            }
            else
            {
...
void ScGridWindow::PagebreakMove( const MouseEvent& rMEvt, BOOL bUp, BOOL bBegin )
{
	...
	if ( 1 )
	{
		if ( bBreak )
		{
			...
			if ( nNew != nPagebreakBreak )
			{
				...
				if (1)		// sonst in AdjustPrintZoom schon passiert
				{
