...
BEGIN
{
	...
	%unexpandable_files =();
	$unexpandable_files{"so_activex.dll"} = 1; # cabinet file cannot be extracted
	@skipped_files = ();
...
sub copy_files_into_directory_structure
{
	...
	for ( my $i = 1; $i <= $maxsequence; $i++ )
	{
		if ( exists($fileorder->{$i}) )
		{
			...
			# expand.exe seems to have a problem with cabinet files, that contain only one file. In this case
			# it is possible, that the cabinet file exists surprisingly in the destination directory.
			# This files are named in "unexpandable_files".
				
			if ( ! -f $sourcefile )
			{			
				if ( exists($unexpandable_files{$file}) )
				{
					print "Warning: Skipping file \"$destfile\" because of problem with expand.exe!\n";
					push(@skipped_files, $destfile);
					next;
				}
			}
...
if ( $#skipped_files > -1 ) { foreach my $file (@skipped_files) { print "Warning: Skipped file: $file\n"; } }
