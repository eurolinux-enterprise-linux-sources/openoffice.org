...
//	pViewData->GetViewShell()->GetViewFrame()->GetWindow().GrabFocus();
	...
	else
	{
		...
		if (nPagebreakMouse)
		{
			...
			// CaptureMouse();
			StartTracking();
			PagebreakMove( rMEvt, FALSE, TRUE );
			return;
		}
	}
    ...
    if ( !bDouble && !bFormulaMode && rMEvt.IsLeft() )
	{
		...
		if (pAttr->HasButton())
		{
			DoPushButton( nPosX, nPosY, rMEvt );	// setzt evtl. bPivotMouse
			return;
		}
...
void __EXPORT ScGridWindow::MouseButtonUp( const MouseEvent& rMEvt )
{
	...
	if (nPagebreakMouse)
	{
		PagebreakMove( rMEvt, TRUE, FALSE );
		nPagebreakMouse = SC_PD_NONE;
		SetPointer( Pointer( POINTER_ARROW ) );
		ReleaseMouse();
		return;
...
//								"Block falsch fuer AutoFill" );
		...
		{
            pViewData->GetFillData( nStartCol, nStartRow, nEndCol, nEndRow );
			USHORT nStartFillCol = pViewData->GetRefStartX();
            USHORT nStartFillRow = pViewData->GetRefStartY();
			USHORT nEndFillCol = pViewData->GetRefEndX();
			USHORT nEndFillRow = pViewData->GetRefEndY();
			bool bUnchanged = false;
            if (nEndFillCol == nEndCol && nEndFillRow == nEndRow && 
				nStartFillCol == nStartCol && nStartFillRow == nStartRow)
			{ 
				bUnchanged = true;
            }
			if (!bUnchanged)
			{
                ...
                ScRange aBlockRange( nStartFillCol, nStartFillRow, nTab, nEndFillCol, nEndFillRow, nTab );
				pView->MarkRange( aBlockRange, FALSE );

				pViewData->GetDispatcher().Execute( FID_FILL_SERIES, SFX_CALLMODE_SLOT | SFX_CALLMODE_RECORD );
			}
		}
	}
...
void __EXPORT ScGridWindow::MouseMove( const MouseEvent& rMEvt )
{
	...
	if (nPagebreakMouse)
	{
		PagebreakMove( rMEvt, FALSE, FALSE );
		return;
	}
...
void ScGridWindow::UpdateDragRectOverlay()
{
    ...
    if ( bDragRect || bPagebreakDrawn )
    {
        ...
        if (bPagebreakDrawn)
        {
            aRect = Rectangle ( aScrPos.X(), aScrPos.Y(),
                         aScrPos.X() + ( nSizeXPix ) * nLayoutSign, aScrPos.Y() + nSizeYPix );
        }
        else
        {
        ...
        if (bPagebreakDrawn)
             aPixelRects.push_back( aRect );
        else if ( meDragInsertMode == INS_CELLSDOWN )
        {
		...
		if(pOverlayManager)
		{
            ...
            sdr::overlay::OverlayType eType = bPagebreakDrawn ? sdr::overlay::OVERLAY_TRANSPARENT : sdr::overlay::OVERLAY_INVERT;
            const svtools::ColorConfig& rColorCfg = SC_MOD()->GetColorConfig();
            Color aColor(COL_BLACK);
            if (bPagebreakDrawn)
                aColor = rColorCfg.GetColorValue(svtools::CALCPAGEBREAKMANUAL).nColor;
